...
class Hacker:
    ...
    def __get_unencrypted_assets(self) -> list[Asset]:
        ...
        return [asset for asset in self.__inventory if not asset.get_encrypted]
    ...
    def retrieve_asset(self, name: str) -> Asset | None:
        ...
        if len(assets) == 0:
            print(
                f"{self.__name} fails to retrieve {name} from inventory; the {name} must be encrypted or does not exist.")
            return None
        else:
            asset = assets[0]
    ...
    def acquire_rig(self, rig: Rig, password: str) -> None:
        ...
        if self.__rig is not None:
            print(
                f"{self.__name} fails to acquire a rig; {self.__name} cannot bring themself to replace {self.__rig.name}.")
        elif not isinstance(rig, Rig):
            print(
                f"{self.__name} fails to acquire a rig; the object they thought was a rig was actually something else...")
        elif rig.registered:
            ...
            if crypto_token is None:
                print(
                    f"{self.__name} fails to acquire a rig; a rig costs 1 CryptoToken and {self.__name} can't find any.")
            else:
                self.__rig = rig
                print(f"{self.__name} acquires {self.__rig.name} for 1 CryptoToken. Hooray!")
